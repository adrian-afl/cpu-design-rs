module uart_tx #(
  ...
  always @(*) begin
    case (state)
      ...
      S_START:
      if (bit_tick) next_state = S_SEND_BYTE;
      else next_state = S_START;
      S_SEND_BYTE:
      if (bit_tick && bit_cnt == 3'd7) next_state = S_STOP;
      else next_state = S_SEND_BYTE;
      S_STOP:
      if (bit_tick) next_state = S_IDLE;
...
*/
  ...
  always @(posedge clk or negedge rst_n) begin
    if (rst_n == 1'b0) begin
      tx_data_latch <= 8'd0;
    end else if (accept) tx_data_latch <= tx_data;
...
*/
  ...
  always @(posedge clk or negedge rst_n) begin
    ...
    else if (state == S_IDLE) cycle_cnt <= 'd0;
    else if (bit_tick) cycle_cnt <= 'd0;
